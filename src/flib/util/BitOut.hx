...
class BitOut{
    ...
    private inline function writeBits(val:Int32,num:Int){
        for(i in 0...num){
            var posa=7-bit;
            var pos2=num-i-1;
            var bitval=(Int32.toNativeInt((Int32.ushr((Int32.and(val,(Int32.shl((Int32.ofInt(1)),pos2)))),pos2))));
            cbyte|=bitval<<posa;
            if(++bit==8){
                o.writeByte(cbyte);
                writenBytes++;
    ...
    public inline function flushBits(){
        if(bit!=0)writeBits((Int32.ofInt(0)),8-bit);
        writenBytes=0;
    }
    ...
    public inline function writeUnsigned(val:Int32,?nb:Int=0){
        if(nb==0)nb=BitFields.bits_unsigned(val);
        writeBits(val,nb);
    }
    public inline function writeSigned(val:Int32,?nb:Int=0){
    ...
    public inline function encWriteUnsigned30(v:Int)encWriteUnsigned((Int32.ofInt(v&0x3fffffff)))public inline function encWriteUnsigned(v:Int32){
        encWriteSigned(v);
    }
    public inline function encWriteSigned(bv:Int32){
        var e=(Int32.toNativeInt((Int32.ushr(bv,28))));
        var n=(Int32.toNativeInt((Int32.and(bv,(Int32.ofInt((1<<28)-1))))));
        var d=(n>>21)&0x7F;
        var c=(n>>14)&0x7F;
        var b=(n>>7)&0x7F;
        var a=n&0x7F;
